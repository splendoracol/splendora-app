...
export function confirmationEmailHtml({ orderNumber, customerName, items, total, address, city, phone }) {
  ...
  const itemsHtml = (items || []).map(it => {
    ...
    const photoBox = photoUrl
      ? `<img src="${escapeHtml(photoUrl)}" alt="${escapeHtml(it.name || '')}" width="80" height="100" style="width: 80px; height: 100px; object-fit: cover; border-radius: 4px; display: block;" />`
      : `<div style="background: linear-gradient(135deg, #FDF2F8, #FAF8F5); width: 80px; height: 100px; border-radius: 4px;"></div>`;
    return `
