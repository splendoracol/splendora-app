...
const TABS=[{id:'dashboard',label:'Inicio'},{id:'inventory',label:'Inventario'},{id:'orders',label:'Pedidos'},{id:'finances',label:'Finanzas'},{id:'catalog',label:'Catálogo'},{id:'tools',label:'Más'}];
...
const STATUS={pending:{label:'Pendiente',color:'#D4A843'},confirmed:{label:'Confirmado',color:'#4A6FA5'},shipped:{label:'Enviado',color:'#7B9ECF'},delivered:{label:'Entregado',color:'#4A9E6B'},cancelled:{label:'Cancelado',color:'#C0504E'}};
...
function genCode(cat,idx){const p={Blusas:'BL',Pantalones:'PN',Vestidos:'VS',Faldas:'FL',Conjuntos:'CJ',Accesorios:'AC',Zapatos:'ZP',Bolsos:'BO',Otro:'OT'};return`SPL-${p[cat]||'SP'}-${String(idx).padStart(4,'0')}`;}
...
function buildExcel(products,orders,expenses,config,month,year){
  ...
  const done=fo.filter(o=>o.status==='delivered'),rv=done.reduce((a,o)=>a+(o.total||0),0),cs=done.reduce((a,o)=>a+(o.cost_total||0),0),ex=fe.reduce((a,x)=>a+(x.amount||0),0);
  const nt=rv-cs-ex,biz=nt*0.1,dist=nt-biz,s1=dist*0.5,s2=dist*0.5;
...
function dlExcel(p,o,e,c,month,year){const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([buildExcel(p,o,e,c,month,year)],{type:'application/vnd.ms-excel'}));a.download=`SPLENDORA_${new Date().toISOString().slice(0,10)}.xls`;a.click();}
...
function Modal({open,onClose,title,children,wide}){
  ...
  return<div className="modal-overlay" onClick={onClose}><div className={`modal-content ${wide?'modal-wide':''}`} onClick={e=>e.stopPropagation()}>
    ...
    </div><div style={{padding:'0 22px 22px'}}>{children}</div>
  </div></div>;
}
...
function MonthSelector({month,year,onChange}){
  const now=new Date();
  return<div style={{display:'flex',gap:6,marginBottom:14,overflowX:'auto',paddingBottom:4}}>
    <button className="neu-btn neu-btn-sm" onClick={()=>onChange(null,now.getFullYear())} style={{whiteSpace:'nowrap',fontSize:10,...(month===null?{boxShadow:'var(--pressed)',color:'#4A6FA5',fontWeight:800}:{})}}>Todo</button>
    {MONTHS.map((m,i)=>(
      <button key={i} className="neu-btn neu-btn-sm" onClick={()=>onChange(i,year)} style={{whiteSpace:'nowrap',fontSize:10,...(month===i?{boxShadow:'var(--pressed)',color:'#4A6FA5',fontWeight:800}:{})}}>{m.slice(0,3)}</button>
    ))}
  </div>;
}
...
export default function HomePage(){
  ...
  async function updateOrderStatus(id,status,items,prev){
    ...
    if(status==='delivered'&&prev!=='delivered'){for(const it of(items||[])){const p=products.find(x=>x.id===it.productId);if(p)await supabase.from('products').update({stock:Math.max(0,p.stock-it.qty)}).eq('id',p.id);}}
    loadAll();
  }
  ...
  // Filtered by month
  const filteredOrders=useMemo(()=>fMonth!==null?orders.filter(o=>{const d=new Date(o.created_at);return d.getMonth()===fMonth&&d.getFullYear()===fYear;}):orders,[orders,fMonth,fYear]);
  const filteredExpenses=useMemo(()=>fMonth!==null?expenses.filter(x=>{const d=new Date(x.created_at);return d.getMonth()===fMonth&&d.getFullYear()===fYear;}):expenses,[expenses,fMonth,fYear]);
  ...
  const m=useMemo(()=>{
    ...
    const ex=filteredExpenses.reduce((s,e)=>s+(e.amount||0),0),nt=rv-cs-ex;
    const biz=nt*0.10,dist=nt-biz,s1=dist*0.5,s2=dist*0.5;
    return{ic,ir,dn,rv,cs,ex,nt,biz,s1,s2,low:products.filter(p=>p.stock>0&&p.stock<=2),out:products.filter(p=>p.stock===0),pnd:filteredOrders.filter(o=>o.status==='pending'||o.status==='confirmed')};
  ...
  return(
    <div style={{minHeight:'100vh',background:'#F0F2F5',fontFamily:"'Montserrat',sans-serif",color:'#1A1D23',paddingBottom:78}}>
      ...
      <div style={{padding:'0 14px'}}>
        ...
        {/* DASHBOARD */}
        {tab==='dashboard'&&<div>
          <MonthSelector month={fMonth} year={fYear} onChange={(m,y)=>{setFMonth(m);setFYear(y);}}/>
          ...
          <div className="neu-card" style={{marginTop:14}}>
            ...
            <div style={{display:'flex',gap:8}}>
              {[{n:'SPLENDORA',p:'10%',v:m.biz,c:'#4A6FA5'},{n:config.partner1,p:'45%',v:m.s1,c:'#1A1D23'},{n:config.partner2,p:'45%',v:m.s2,c:'#1A1D23'}].map((x,i)=>
          ...
          {products.filter(p=>!search||p.name.toLowerCase().includes(search.toLowerCase())||(p.code||'').toLowerCase().includes(search.toLowerCase())).map(p=>(
            <div key={p.id} className="neu-card" style={{padding:12,display:'flex',gap:10,alignItems:'center',marginBottom:8}}>
              ...
              <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:5,flexShrink:0}}>
                <div className="neu-card neu-pressed" style={{padding:'3px 10px',borderRadius:8}}><span style={{fontSize:13,fontWeight:800,color:p.stock===0?'#C0504E':p.stock<=2?'#D4A843':'#4A9E6B'}}>{p.stock}</span></div>
        ...
        {tab==='orders'&&<div>
            ...
            <div key={o.id} className="neu-card" style={{padding:14,marginBottom:8}}>
              ...
              <div style={{display:'flex',gap:5,marginTop:10,flexWrap:'wrap'}}>
                {Object.entries(STATUS).filter(([k])=>k!=='cancelled').map(([k,v])=>
                  <button key={k} className="neu-btn neu-btn-sm" onClick={()=>updateOrderStatus(o.id,k,o.items,o.status)} style={{padding:'4px 9px',fontSize:9,...(o.status===k?{boxShadow:'var(--pressed)',color:v.color,fontWeight:800}:{})}}>{v.label}</button>
                )}
          ...
          {filteredOrders.length===0&&<div className="neu-card" style={{textAlign:'center',padding:32,color:'#9CA3AF'}}>Sin pedidos en este periodo</div>}
        </div>}
        ...
        {tab==='finances'&&<div>
          ...
          <MonthSelector month={fMonth} year={fYear} onChange={(mm,y)=>{setFMonth(mm);setFYear(y);}}/>
          <div className="neu-card" style={{padding:18,marginBottom:14}}>
            ...
            <div style={{display:'flex',gap:8,marginTop:14}}>
              {[{n:'SPLENDORA',p:'10%',v:m.biz,c:'#4A6FA5'},{n:config.partner1,p:'45%',v:m.s1},{n:config.partner2,p:'45%',v:m.s2}].map((x,i)=>
          ...
          {filteredExpenses.map(e=><div key={e.id} className="neu-card" style={{padding:10,display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
          ...
          {filteredExpenses.length===0&&<div className="neu-card neu-pressed" style={{textAlign:'center',padding:24,color:'#9CA3AF',fontSize:12}}>Sin gastos en este periodo</div>}
        </div>}
        ...
        {tab==='catalog'&&<div>
          ...
          <p style={{fontSize:10,color:'#6B7280',marginBottom:12}}>Configura logo, banner y promociones.</p>
...
function ProductForm({initial,onSave}){
  const[f,setF]=useState(initial?{
    ...
    description:initial.description,photo_url:initial.photo_url,photo_url_2:initial.photo_url_2||'',
    discount:initial.discount||0,hide_price:initial.hide_price||false,
  }:{name:'',category:'Blusas',size:'M',sizes:[],color:'',cost_product:0,cost_bag:0,cost_shipping:0,price:0,stock:1,description:'',photo_url:'',photo_url_2:'',discount:0,hide_price:false});
  ...
  async function handleUpload(field,e){
    ...
    try{const url=await uploadPhoto(file);setF(prev=>({...prev,[field]:url}));}catch(err){alert('Error: '+err.message);}
    setUploading(false);
  }
  ...
  return<div>
    ...
    <div style={{marginBottom:16}}>
      <label className="label">Fotos del producto (máx 2)</label>
      <div style={{display:'flex',gap:10}}>
        {[{field:'photo_url',ref:ref1,label:'Principal'},{field:'photo_url_2',ref:ref2,label:'Secundaria'}].map(({field,ref,label})=>(
          <div key={field} style={{flex:1}}>
            <div style={{fontSize:9,color:'#6B7280',marginBottom:4,textAlign:'center'}}>{label}</div>
            <div onClick={()=>ref.current?.click()} style={{width:'100%',height:80,borderRadius:12,boxShadow:f[field]?'var(--raised-sm)':'var(--pressed)',cursor:'pointer',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center'}}>
              {f[field]?<img src={f[field]} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<div style={{textAlign:'center',color:'#9CA3AF',fontSize:10}}>{uploading?'...':'+ Foto'}</div>}
            </div>
            <input ref={ref} type="file" accept="image/*" style={{display:'none'}} onChange={e=>handleUpload(field,e)}/>
            {f[field]&&<button className="neu-btn neu-btn-sm" style={{width:'100%',marginTop:4,fontSize:9}} onClick={()=>setF({...f,[field]:''})}>Quitar</button>}
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{marginBottom:16}}>
      ...
      <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
        {SIZES_LIST.map(s=><button key={s} type="button" className="neu-btn neu-btn-sm" onClick={()=>setF(prev=>({...prev,sizes:prev.sizes.includes(s)?prev.sizes.filter(x=>x!==s):[...prev.sizes,s]}))} style={{
...
function OrderForm({products,onSave}){
  ...
  return<div>
    ...
    <div style={{display:'flex',gap:6,marginBottom:12}}>
      ...
      <button className="neu-btn neu-btn-accent" onClick={()=>{const p=products.find(x=>x.id===sel);if(!p||f.items.find(i=>i.productId===p.id))return;setF({...f,items:[...f.items,{productId:p.id,name:p.name,code:p.code,qty,priceUnit:p.price,costUnit:p.cost_total,subtotal:p.price*qty}]});setSel('');setQty(1);}} style={{padding:'10px 14px'}}>+</button>
    </div>
...
function CatCfgForm({cfg,onSave}){
  ...
  const[uploading,setUploading]=useState(false);
  const logoRef=useRef(null),bannerRef=useRef(null);

  async function handleUpload(field,e){
    const file=e.target.files?.[0];if(!file)return;setUploading(true);
    try{setC(prev=>({...prev,[field]:await uploadPhoto(file)}));}catch(err){alert('Error: '+err.message);}
    setUploading(false);
  }
  ...
  return<div>
    ...
    <div style={{marginBottom:16}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:14}}>
        <div onClick={()=>logoRef.current?.click()} style={{width:70,height:70,borderRadius:12,boxShadow:c.logo_url?'var(--raised-sm)':'var(--pressed)',cursor:'pointer',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center'}}>
          {c.logo_url?<img src={c.logo_url} alt="" style={{width:'100%',height:'100%',objectFit:'contain'}}/>:<div style={{textAlign:'center',color:'#9CA3AF',fontSize:9}}>{uploading?'...':'+ Logo'}</div>}
        </div>
    ...
    <Fld label="Banner activo">
      <button type="button" className="neu-btn" style={{width:'100%',...(c.banner_active?{background:'#4A6FA5',color:'#FFF'}:{})}} onClick={()=>setC({...c,banner_active:!c.banner_active})}>
        {c.banner_active?'Sí — Visible':'No — Oculto'}
      </button>
    </Fld>
    <Fld label="Texto del banner"><input className="neu-input" value={c.banner_text} onChange={e=>setC({...c,banner_text:e.target.value})} placeholder="🔥 20% OFF en toda la colección"/></Fld>

    {/* BANNER IMAGE */}
    <div style={{marginBottom:16}}>
      <label className="label">Imagen del banner (opcional)</label>
      <div style={{display:'flex',alignItems:'center',gap:14}}>
        <div onClick={()=>bannerRef.current?.click()} style={{width:140,height:60,borderRadius:10,boxShadow:c.banner_image?'var(--raised-sm)':'var(--pressed)',cursor:'pointer',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center'}}>
          {c.banner_image?<img src={c.banner_image} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<div style={{color:'#9CA3AF',fontSize:10}}>{uploading?'...':'+ Imagen'}</div>}
        </div>
