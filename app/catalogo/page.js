...
export default function CatalogoPage() {
  ...
  function getPhotos(p) {
    ...
    if (p.photo_url_2) photos.push(p.photo_url_2);
    return photos;
  }
  ...
  if (loading) return (
    <div style={{minHeight:'100vh',background:'#F0F2F5',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Montserrat',sans-serif"}}>
      <div style={{textAlign:'center'}}>
        {logo ? <img src={logo} alt="SPLENDORA" style={{width:60,marginBottom:8}} /> : null}
  ...
  return (
    <div style={{minHeight:'100vh',background:'#F0F2F5',fontFamily:"'Montserrat',sans-serif",color:'#1A1D23'}}>
      ...
      <div style={{background:'#FFF',padding:'20px 20px 14px',textAlign:'center',boxShadow:'0 2px 8px rgba(0,0,0,0.04)'}}>
        {logo && <img src={logo} alt="SPLENDORA.COL" style={{width:55,height:55,objectFit:'contain',marginBottom:6}}/>}
        ...
        {filtered.length===0?(
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(165px,1fr))',gap:14}}>
            {filtered.map(p=>{
              ...
              const curPhoto=photoIdx[p.id]||0;
              ...
              return(
                <div key={p.id} style={{background:'#F0F2F5',borderRadius:16,overflow:'hidden',boxShadow:'5px 5px 10px #D1D3D6,-5px -5px 10px #FFFFFF',position:'relative'}}>
                  ...
                  {/* PHOTO VIEWER */}
                  <div style={{height:180,display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',margin:8,borderRadius:12,boxShadow:'inset 3px 3px 6px #D1D3D6,inset -3px -3px 6px #FFFFFF',position:'relative'}}>
                    {photos.length>0?(
                      <>
                        <img src={photos[curPhoto]||photos[0]} alt={p.name} style={{width:'100%',height:'100%',objectFit:'cover',borderRadius:12}}/>
                        {photos.length>1&&(
                          <div style={{position:'absolute',bottom:6,left:0,right:0,display:'flex',justifyContent:'center',gap:4}}>
                            {photos.map((_,i)=>(
                              <button key={i} onClick={(e)=>{e.stopPropagation();setPhotoIdx(prev=>({...prev,[p.id]:i}));}}
                                style={{width:8,height:8,borderRadius:'50%',border:'none',cursor:'pointer',
                                  background:(curPhoto===i)?'#1A1D23':'rgba(255,255,255,0.7)',
                                  boxShadow:'0 1px 3px rgba(0,0,0,0.3)'}}/>
                            ))}
                          </div>
                        )}
                      </>
                    ):<span style={{fontSize:44,color:'#D1D3D6'}}>+</span>}
                  </div>
      ...
      <div style={{textAlign:'center',padding:'24px 20px 32px',background:'#FFF',marginTop:20}}>
        {logo&&<img src={logo} alt="SPLENDORA.COL" style={{width:45,height:45,objectFit:'contain',marginBottom:8}}/>}
